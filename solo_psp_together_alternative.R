...
two_component_model <- function(cmd = c("graph", "Q", "mu", "initial", 
  ...
  prior.v_b_r <- function(v_b_r=feed_x){ #around 50
    ...
    return(dnorm(v_b_r,  mean  = 50.0,    sd   = 7.5, log=TRUE))
  }
...
result = inla(flux ~ -1 + f(idx, model = rgen) + f(sc_id, model="iid"),
              ...
              ,int.strategy="eb" #int.strategy="grid"  #int.strategy="ccd"
              #,strategy="adaptative" # ,strategy="gaussian"
              ),
              safe = TRUE,
              verbose = TRUE)
